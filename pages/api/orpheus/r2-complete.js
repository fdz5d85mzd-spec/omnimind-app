...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rows = await sql`SELECT t.upload_key_hash, t.expires_at, f.pathname, f.r2_upload_id, f.storage_provider
    ...
    const scheme=file.storage_provider==='hetzner'?'hetzner':'r2';
    await sql`UPDATE transfer_files SET blob_url = ${`${scheme}://${file.pathname}`}, uploaded_at = NOW() WHERE id = ${fileId} AND transfer_id = ${transferId}`;
    return json(res, 200, { id: fileId, pathname: file.pathname, url: `${scheme}://${file.pathname}` });
  } catch (error) {
