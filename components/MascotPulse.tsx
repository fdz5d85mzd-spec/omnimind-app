...
export default function MascotPulse(){
 ...
 return <div className="fixed bottom-24 right-3 z-[65] sm:bottom-5 sm:right-5" dir={lang==="ar"?"rtl":"ltr"}>{open&&<aside className="mb-3 w-[min(330px,calc(100vw-1.5rem))] overflow-hidden rounded-3xl border border-cyan/20 bg-[#090d25]/95 shadow-[0_22px_80px_rgba(0,0,0,.55)] backdrop-blur-2xl"><div className="relative bg-gradient-to-br from-accent/25 via-cyan/10 to-transparent p-4"><button onClick={()=>setOpen(false)} className="absolute right-3 top-3 rounded-full p-1.5 text-muted hover:bg-white/10 hover:text-white" aria-label={c.close}><X size={15}/></button><div className="flex items-center gap-3 pr-7"><Image src="/helen/creatures/v2/stage-2.png" alt="OmniMind mascot" width={62} height={62} className="h-16 w-16 object-contain drop-shadow-[0_0_18px_rgba(34,211,238,.45)]"/><div><p className="text-[10px] font-bold tracking-[.16em] text-cyan">OMNI PULSE</p><h2 className="mt-1 text-sm font-bold leading-snug">{c.hello}</h2></div></div><p className="mt-3 text-xs leading-relaxed text-muted">{article&&article.language===lang?article.title:c.news}</p><Link href={storyHref} onClick={()=>setOpen(false)} className="mt-3 flex items-center justify-between rounded-xl bg-white/[.06] px-3 py-2.5 text-xs font-semibold hover:bg-white/[.1]"><span className="flex items-center gap-2"><Bell size={14} className="text-amber"/>{c.read}</span><ChevronRight size={14}/></Link><Link href={destination.href} onClick={()=>setOpen(false)} className="mt-2 flex items-center justify-between rounded-xl border border-cyan/15 px-3 py-2.5 text-xs text-muted hover:text-white"><span>{c.try}: <b>{destination.name}</b></span><ChevronRight size={14}/></Link></div></aside>}<button onClick={()=>setOpen(v=>!v)} className="relative grid h-14 w-14 place-items-center rounded-2xl border border-cyan/25 bg-[#0b1133]/95 shadow-[0_12px_35px_rgba(0,0,0,.45)] backdrop-blur-xl transition hover:-translate-y-1" aria-label="Omni Pulse"><span className="absolute -right-1 -top-1 h-3 w-3 rounded-full border-2 border-[#090d25] bg-amber animate-pulse"/><Image src="/helen/creatures/v2/stage-2.png" alt="" width={45} height={45} className="h-11 w-11 object-contain"/></button></div>;
}
