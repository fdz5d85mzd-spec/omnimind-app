...
function RadarIcon() {
  return (
    ...
    >
      ...
      <path
        d="M12 12 L12 4.5 A7.5 7.5 0 0 1 19.5 12 Z"
        fill="currentColor"
        stroke="none"
        opacity="0.35"
      />
  ...
  return (
    ...
    >
      <div className="p-3">
        ...
        <Link
          href="/orpheus"
          className="mt-1 flex items-center gap-2.5 rounded-xl px-3.5 py-2.5 text-sm font-medium text-muted hover:bg-white/[0.04] hover:text-white transition-colors"
        >
          <RadarIcon />
          Orpheus
        </Link>
      ...
      <nav className="flex-1 overflow-y-auto px-3 pb-3 space-y-0.5">
        {conversations.length > 0 && (
          <div className="flex items-center justify-between px-2.5 pt-2 pb-1.5">
            <p className="text-[11px] font-semibold tracking-wide text-mutedDark uppercase">
              Recent
            </p>
            <button
              onClick={() => {
            ...
            {user ? (
              <>
                <div>
                  ...
                  <p className="text-[11px] text-mutedDark truncate">
                    {user.email}
                  </p>
                </div>
                {privileged ? (
                  <div className="flex items-center justify-between rounded-lg bg-white/[0.03] border border-white/[0.06] px-2.5 py-2">
                    ...
                    <span className="text-[10px] text-cyan uppercase">
                      Admin
                    </span>
                  </div>
                ) : (
                  credits && (
                    <div className="flex items-center justify-between rounded-lg bg-white/[0.03] border border-white/[0.06] px-2.5 py-2">
                      ...
                      <span className="text-[10px] text-mutedDark uppercase">
                        {credits.plan}
                      </span>
                    </div>
                  )
                ...
                <p className="text-xs text-muted leading-relaxed">
                  Conversation history is still stored on this device only —
                  accounts don&apos;t sync it across devices yet.
                </p>
                <Link
                  href="/settings"
              ...
              <>
                ...
                <p className="text-xs text-muted leading-relaxed">
                  History is stored on this device only. Sign in to unlock
                  account features as they ship.
                </p>
                <Link
                  href="/login"
                  className="block text-xs font-semibold text-cyan hover:text-white transition-colors"
                >
                  Sign in →
                </Link>
              </>
