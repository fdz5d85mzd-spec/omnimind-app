...
const fileTypeLabel = (name) => {
  ...
  return extension.replace(/[^a-z0-9]/gi, '').slice(0, 3).toUpperCase() || 'FILE';
};
...
function addFiles(selected) {
  ...
  incoming.forEach((file) => {
    if (!existing.has(`${file.name}-${file.size}-${file.lastModified}`)) files.push(file);
  });
  renderFiles();
}
...
async function uploadFileToR2(file, record, transfer, onProgress) {
  const initResponse = await fetch('/api/orpheus/r2-init', {
    ...
    body: JSON.stringify({ transferId: transfer.id, fileId: record.id, uploadKey: transfer.uploadKey }),
  });
  const init = await initResponse.json();
  if (!initResponse.ok) throw new Error(init.error || 'Could not start upload.');

  const partCount = Math.ceil(file.size / init.partSize);
  const completed = [];
  const batchSize = 20;
  for (let batchStart = 1; batchStart <= partCount; batchStart += batchSize) {
    const partNumbers = Array.from({ length: Math.min(batchSize, partCount - batchStart + 1) }, (_, index) => batchStart + index);
    const signedResponse = await fetch('/api/orpheus/r2-parts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ transferId: transfer.id, fileId: record.id, uploadKey: transfer.uploadKey, uploadId: init.uploadId, partNumbers }),
    });
    const signed = await signedResponse.json();
    if (!signedResponse.ok) throw new Error(signed.error || 'Could not authorize upload.');

    for (let offset = 0; offset < signed.parts.length; offset += 4) {
      const group = signed.parts.slice(offset, offset + 4);
      const results = await Promise.all(group.map(async ({ partNumber, url }) => {
        const start = (partNumber - 1) * init.partSize;
        const end = Math.min(start + init.partSize, file.size);
        const response = await fetch(url, { method: 'PUT', body: file.slice(start, end) });
  ...
  const completeResponse = await fetch('/api/orpheus/r2-complete', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ transferId: transfer.id, fileId: record.id, uploadKey: transfer.uploadKey, uploadId: init.uploadId, parts: completed.sort((a, b) => a.partNumber - b.partNumber) }),
  });
  const complete = await completeResponse.json();
  if (!completeResponse.ok) throw new Error(complete.error || 'Could not finish upload.');
  return complete;
}
...
transferButton.addEventListener('click', async () => {
  ...
  if (mode === 'email' && (!emailTo.validity.valid || !emailFrom.validity.valid || !emailTo.value || !emailFrom.value)) {
  ...
  transferButton.querySelector('span').textContent = 'Preparing secure transfer…';
  ...
  try {
    ...
    if (!response.ok) throw new Error(transfer.error || 'Could not create transfer.');
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      const uploadedBlob = await uploadFileToR2(file, record, transfer, (chunkBytes) => {
        currentFileBytes += chunkBytes;
        const percentage = Math.min(100, Math.round(((completedBytes + currentFileBytes) / totalSize) * 100));
        progressBar.style.width = `${percentage}%`;
        progressPercent.textContent = `${percentage}%`;
      });
      uploadedFiles.push({ id: record.id, pathname: uploadedBlob.pathname, url: uploadedBlob.url });
    ...
    const finalizeResponse = await fetch('/api/orpheus/transfers', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: transfer.id, uploadKey: transfer.uploadKey, files: uploadedFiles }),
    });
...
async function loadDownload() {
  ...
  try {
    ...
    root.querySelector('#download-title').textContent = 'This transfer is unavailable.';
...
async function startCheckout(payload) {
  const button = payload.plan ? root.querySelector(`[data-plan="${payload.plan}"]`) : root.querySelector('#buy-once');
...
root.querySelectorAll('[data-plan]').forEach((button) => {
  if (button.dataset.plan !== 'free') button.addEventListener('click', () => startCheckout({ plan: button.dataset.plan }));
});
root.querySelector('#buy-once').addEventListener('click', () => startCheckout({ gb: Number(range.value) }));
