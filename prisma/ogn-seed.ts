...
async function main() {
  console.log("🌱 Seeding OGN database...");

  // ─── Admin User ────────────────────────────────────────────
  // Random per run, not a guessable default -- this repo is public, so a
  // hardcoded password here would be a live credential sitting in source.
  ...
  console.log(`✓ Admin user: admin@ogn.app / ${adminPasswordPlain}`);
  ...
  // ─── Sources (RSS feeds the Collection agent reads from) ─────
  const sources = [
  ...
  console.log(`✓ ${sources.length} RSS sources`);

  // ─── A few sample articles so the site isn't empty before the ──
  // ─── pipeline has run for the first time ────────────────────
  ...
  console.log(`✓ ${sampleArticles.length} sample articles`);

  // ─── Settings the pipeline agents read ───────────────────────
