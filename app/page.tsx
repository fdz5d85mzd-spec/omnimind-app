...
export default function Landing() {
  ...
  const [report, setReport] =
    useState<Awaited<ReturnType<typeof getOrchestratorReport>>>(null);
  const [fleet, setFleet] =
    useState<Awaited<ReturnType<typeof getFleetStatus>>>(null);
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative min-h-[92vh] flex flex-col items-center justify-center overflow-hidden px-6 pt-24 pb-16">
        ...
        <div className="pointer-events-none absolute inset-x-8 top-[29%] z-[1] mx-auto hidden max-w-[1320px] items-start justify-between lg:flex">
          <aside className="pointer-events-auto w-56 rounded-3xl border border-cyan/15 bg-card/45 p-4 text-left shadow-panel backdrop-blur-xl">
            <p className="text-[10px] font-bold uppercase tracking-[.18em] text-cyan">
              {reachable === false ? "SYSTEM CHECK" : "LIVE NOW"}
            </p>
            ...
            <span className="mt-3 inline-flex items-center gap-2 text-[11px] text-muted">
              <i
                className={`h-2 w-2 rounded-full ${reachable ? "bg-emerald animate-pulseDot" : "bg-amber"}`}
              />
              {reachable ? "OmniMind connected" : "Connecting…"}
            </span>
          </aside>

          <aside className="pointer-events-auto w-56 space-y-2 rounded-3xl border border-violet/20 bg-card/45 p-4 text-left shadow-panel backdrop-blur-xl">
            <p className="mb-3 text-[10px] font-bold uppercase tracking-[.18em] text-violet">
              OMNI SUGGESTS
            </p>
            <Link
              href="/atlas"
              className="block rounded-2xl border border-white/[.06] bg-white/[.04] p-3 transition hover:-translate-y-0.5 hover:bg-white/[.08]"
            >
              <b className="text-sm text-white">Atlas</b>
            ...
            <Link
              ...
              className="block rounded-2xl border border-white/[.06] bg-white/[.04] p-3 transition hover:-translate-y-0.5 hover:bg-white/[.08]"
            >
              <b className="text-sm text-white">Orpheus</b>
              <span className="mt-1 block text-[11px] text-muted">
                Play Signal Run →
              </span>
            </Link>
      ...
      <section className="max-w-6xl mx-auto px-6 py-28">
        <div className="text-center mb-16">
          <p className="text-cyan text-xs font-bold tracking-[0.2em] mb-3">
            {t.featuresEyebrow}
          </p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {subsystems(t).map((s, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`h-10 w-10 rounded-xl ${c.bg} border ${c.border} flex items-center justify-center mb-4 ${c.text}`}
                >
                  <Icon />
                </div>
...
function Stat({
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-[11px] tracking-wide text-mutedDark uppercase">
        {label}
      </p>
    </div>
  );
    ...
    <footer className="border-t border-white/[0.06] px-6 py-10">
      <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <LogoMark size={18} />
          <span className="font-head font-semibold text-sm text-white">
            OmniMind
          </span>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-2">
        {steps.map((s, i) => {
          ...
          return (
            <div
              key={s.label}
              className="flex items-center gap-2 sm:gap-2 w-full sm:w-auto"
            >
              <div
                className={`flex-1 sm:flex-none rounded-xl border ${c.border} ${c.bg} px-5 py-4 text-center min-w-[120px]`}
              >
