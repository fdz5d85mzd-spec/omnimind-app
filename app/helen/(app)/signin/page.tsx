...
function SignInForm() {
  ...
  if (status === "sent") {
    return (
      <div className="flex flex-1 flex-col items-center justify-center text-center">
        <h1 className="mb-2 font-helen-display text-xl font-semibold">{t.checkEmailTitle}</h1>
  ...
  return (
    <div className="flex flex-1 flex-col justify-center">
      <h1 className="mb-2 text-center font-helen-display text-xl font-semibold">{t.signInTitle}</h1>
      <p className="mb-6 text-center text-[13px] text-helen-dim">{t.signInSub}</p>
      <form onSubmit={handleSubmit}>
        ...
        {status === "error" && <p className="mt-2.5 text-center text-xs text-helen-coral">{t.signInError}</p>}
      </form>
    </div>
  );
}
